...
// 调试状态不修改
if (!(profile.run.ch.isDebug || profile.run.mp.isDebug)) {
    DEVICE.savePower();
}
...
var mp2interval = profile.run.mp2.interval;
// 寻车任务间隔，单位为分钟，此处为每100分钟进入寻车
var interval = profile.run.ch.interval;
// 寻车按钮在每日赛事中的位置，从0开始计数，0=特别赛事寻车
var position = profile.run.ch.position;
// 选取上次用车上面/下面
// 由于寻车赛事一般都是2票跑一次，所以每次寻车会运行5次，请选择能够自动拿到奖励，并且是5个油的车，
// 在运行脚本之前请确认：1、最后一次寻车用的必须是这辆车，2、此车满油。3、如果该车在下方，请选择up=0
var up = profile.run.ch.up;

// 前置寻车开关，如果运行脚本时想先跑一轮寻车，请把0改成1
if (profile.run.ch.preEnable) {
    // start carHunt
    for ( let i = 0; i < profile.run.ch.loopCount; i++ ) {
        carHunt.beforeRun(position);
        
        if(!carHunt.chooseCar(up)) {
            break;
        }
        carHunt.run(i);
...
if (profile.run.mp2.preEnable) {
    toastLog("start mp2.");
    // start mp2
    for ( let i = 0; i < profile.run.mp2.loopCount; i++ ) {
        PLAY2.beforeRun();
